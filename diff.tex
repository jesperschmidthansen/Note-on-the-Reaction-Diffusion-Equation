...
function} \index{Function!sawtooth} \index{Sawtooth function}
\begin{equation}
	\saw(x) = x - \lfloor x \rfloor \, ,
\end{equation}
...
limits in points $x=\ldots -2,-1,0,1,2, \ldots$ exist, but are different the function features
\emph{jump discontinuities} \index{Jump discontinuity} in these points. 
...
series. What we mean by a representation in this case will hopefully be clear as we go on. 
...
infinitely differentiable on its domain. The sawtooth function above clearly does not fulfill 
this when $x$ takes integer values, $\ldots -2,-1,0,1,2, \ldots$. 
Even if the function has a Taylor series, this is not very practical choice since the Taylor 
series converges slowly for periodic functions as we saw in the previous Chapter. 
Then, we suggest to represent the function as a series of known periodic functions, that is, 
\begin{equation}
	\label{eq:Fouriereq}
	F(x) = \frac{a_0}{2} + \sum_{n=1}^\infty a_n \cos\left(\frac{n\pi }{L} \, x\right) 
	+ \sum_{n=1}^\infty b_n \sin\left(\frac{n\pi }{L} \, x\right) \, .
\end{equation} 
This is the famous \emph{Fourier series}, \index{Fourier series}\index{Series!Fourier} and the 
coefficients $a_0, a_1, a_2, \ldots$, $b_1, b_2, b_3, \ldots$ are called the 
\emph{Fourier coefficients}\index{Fourier series!coefficients}, and $n$ the \emph{wave number}
\index{Fourier series!wave number}. 
...
\begin{example}
	The general solution in Equation (\ref{eq:2ndorderseries}) is a Fourier series with $a_n = 0$.
\end{example}

\noindent $n=1$ is the \emph{fundamental term} or \emph{fundamental mode} and note that 
...
\begin{eqnarray}
	p = \frac{2L}{n} \, .
\end{eqnarray}
Often the Fourier series is expressed through the \emph{wave vector} \index{Fourier series!wave
vector} $k_n=2\pi/p = n\pi/L$, that is,
\begin{equation}
...
requires a rather in-depth discussion to series convergence which we will avoid. 
For our purpose, it suffices to state that a function can be represented by a Fourier series 
if the function is
\begin{enumerate}
...
$\ldots, -1 < x <0$, $0 < x < 1, \ldots$ Therefore, $\saw$ can be represented by a
Fourier series. That the sawtooth function can be represent by $F$ does not mean that the two
functions are equal in every point in the domain: $\saw(x) \neq F(x)$ 
...
To this end we need to know about \emph{orthogonality} \index{Function!orthogonality} of functions. Let $f$ and $g$ be complex
values functions, $f,g: I \rightarrow \R$ where $I = [a; b]$, then the \emph{inner product}
\index{Inner product} on $I$ is defined as
\begin{eqnarray}
	\label{eq:innerdef}
	\langle f,g\rangle= \int_a^b f(x) \overline{g}(x) \, \d x \, , 
\end{eqnarray}
where $\overline{g}$ is the complex conjugate to $g$. If $f$ and $g$ are real values functions we
simple have $\langle f,g\rangle= \int_a^b f(x) g(x) \, \d x$. We say that $f$ and $g$ are 
\emph{orthogonal} on $I$ if $\langle f,g \rangle = 0$. 

\begin{example}
	Let $f(x)=x$, $g(x)=\cos(x)$, and $h(x) = \sin(x)$. $f$ and $g$ are orthogonal on $I=[-\pi, \pi]$ because
	$\langle f, g\rangle = \int_{-\pi}^{\pi} x \cos(x) \, \d x = 0$, but $f(x)=x$ and $h(x)$ are not 
	orthognal on $I$ because $\langle f, h\rangle = \int_{-\pi}^{\pi} x \sin(x) \, \d x = 2\pi \neq
	0$   
\end{example}

\bigskip

\noindent We shall benefit from the following properties of the sine and cosine functions on
$I=[-L;L]$ 
\begin{enumerate}
...
We need one additional definition: Let $f:S \rightarrow \mathbb{R}$ and let $s_m$ be the partial sum 
\begin{eqnarray}
	s_m(x) = \frac{a_0}{2} + \sum_{n=1}^m a_n \cos\left(\frac{n\pi}{L} x\right) + 
	\sum_{n=1}^m b_n \sin\left(\frac{n\pi}{L} x\right) \, .
\end{eqnarray}
We say that the Fourier series \emph{converges uniformly} \index{Convergence!uniform} on $S$ if for all $\epsilon > 0$ 
there exists an $N(\epsilon) \in \mathbb{N}$ such that $|s_m(x) - f(x)| < \epsilon$ for all 
$x \in S$ and $m \geq N$. Notice, that $N$ is independent of $x$. Uniform convergence entails that
all points $x \in S$ converges with same upper bound $N$, hence, $N$ only depends on $\epsilon$ not $x$. 
This is a stronger condition compared to pointwise convergence where $N$ in general is a function of
$x$. 

We are now ready for the section's main theorem
\begin{theorem}
	If the Fourier series representation, $F$, of $f:\mathbb{R} \rightarrow \mathbb{R}$ 
	converges uniformly to $f$ on $\mathbb{R}$ and $f=F$ then 
	\begin{description}[style=unboxed,leftmargin=0.5cm]
		\item[1] $\langle f(x), 1 \rangle = La_0$
	...
	equations.}  \index{Fourier series!coefficients} \index{Euler-Fourier equations}
\end{theorem}
...
\begin{proof}
	...
	\item  Multiplying Equation (\ref{eq:f=F}) with $\cos(m \pi x/L)$, $m=1,2,3, \ldots$ and integrate from $-L$ to $L$
		\begin{eqnarray}
			...
			\label{eq:pointii}
		\end{eqnarray}
		The first term on the right-hand side is zero. For the second term 	we again interchange 
		the integration and summation, giving raise to integrals with relations
		\begin{eqnarray}
			\int_{-L}^L \cos\left(\frac{m\pi }{L} \, x\right) \cos\left(\frac{\pi n}{L} \, 
			x\right) \d x = L \delta_{mn} \ ,
		\end{eqnarray}
		which comes from the orthogonality properties above. $\delta_{mn}$ is the Kronecker delta, 
		...
		Therefore, Equation (\ref{eq:pointii}) simplifies to
		\begin{equation}
		...
		\end{equation}
		Notice that if we allow $n=0$ we arrive at Equation (\ref{eq:a0}). This explains the choice 
...
\begin{example}
	...
	\noindent $a_0$: We use $L=1$, then from Equation (\ref{eq:a0}).
	\begin{equation}
		a_0 = \int_{-1}^1 i\saw(x) \, \d x = \int_{-1}^0 x + 1\, \d x + \int_{0}^1 x \, \d x = 1 \ .  
	\end{equation}
	

	\noindent $a_n$: Rather than simply embarking on the tedious task of integrating Equation 
	(\ref{eq:an}) we can re-arrange the integral, which leaves the problem straight forward. 
	Substituting for $\saw$ we get 
	\begin{eqnarray}
		\int_{-1}^1 \saw(x) \cos(n\pi x) \, dx &=& \int_{-1}^0 (x+1) \cos(n\pi x) \, \d x + \int_{0}^1 
			x\cos(n\pi x) \, \d x \nonumber \\
	...
	\noindent $b_n$: Following the protocol above we have  
		...
		\end{equation}
		we then get $\int_{-1}^1 x \sin (n\pi x) = -2(-1)^n/n\pi$. The second integral in Equation 
		...
		\begin{equation}
			\label{eq:fouriersaw}
			\saw(x) \sim \frac{1}{2} - \sum_{n=1}^\infty  \frac{1 + (-1)^n}{n \pi} \sin(n\pi x) 
		\end{equation}
		Note that when $n$ is odd the nominator in the summation is zero, and we can simply 
		let $n$ run over the even natural numbers.
\end{example}

\noindent From Equation (\ref{eq:fouriersaw}) we see that for $x \in \Z$ the Fourier representation yields 
...
infinitely many terms. Rather we plot the partial sum
\begin{eqnarray}
	s_m(x) = \frac{1}{2} - \sum_{n=1}^m  \frac{1 + (-1)^n}{n \pi} \sin(n\pi x) \, .
\end{eqnarray}
We can define the remainder $R_m(x) = \saw(x) - s_m(x)$. The remainder $R_{50}$ is plotted in Fig. 
\ref{fig:remainder-fourier} (a) for the sawtooth function. 
e see a large disagreement in neighbourhoods around 
the points of discontinuities, which is manifested as large amplitude oscillations. This is known 
as the \emph{Gibbs phenomenon}\index{Gibbs phenomenon}. 
...
\begin{figure}[h!]
	\begin{center}
		...
		\caption{\label{fig:remainder-fourier}
			...
			(b) The Fourier coefficient, $b_n$, as function of $n$.
		}
	\end{center}
\end{figure}
...
\begin{exerciseregion}
	\begin{exercise}
		\begin{enumerate}
			\item Proof that Equation (\ref{eq:innerdef}) defines an inner product. (You may have 
	...
	\begin{exercise}
		Let $f:\R \rightarrow \R$ and let the Fourier representation of $f$ be uniform convergent
		to $f$. Proof the Euler-Fourier equation $ \langle f(x),
	...
	\begin{exercise} \label{ex:fourierevenodd}
		...
		even resulting in the \emph{cosine series} \index{Series!cosine}
		\begin{eqnarray}
			\frac{a_0}{2} + \sum_{n=1}^\infty a_n \cos\left(\frac{n\pi}{L}x\right) \, .
		\end{eqnarray}
		If $f$ is odd the Fouier series is odd giving the \emph{sine series}\index{Series!sine}
		\begin{eqnarray}
			\sum_{n=1}^\infty b_n \sin\left(\frac{n\pi}{L}x\right) \, .
		\end{eqnarray}
...
\section{The diffusion equation with Dirichlet boundaries}
After Section \ref{sect:fourier} we are ready to explore the diffusion equation
\begin{eqnarray}
...
\begin{eqnarray}
	c(x,t) = X(x)T(t) \, .
\end{eqnarray}
This is referred to as \emph{separation of variables}\index{Separation of variables}. Of course, factorizing a function in this manner
...
The constant must be the same, and we have
\begin{eqnarray}
...
\end{eqnarray}
We need the boundary conditions to continue. From Equation (\ref{eq:bcstart}) 
\begin{eqnarray}
	c(0, t) = X(0)T(t) = 0 \ \Rightarrow X(0) = 0\, ,
\end{eqnarray}
because $T \neq 0$.  The same is true for $x=L$, and we have Dirichlet BCs $X(0)=X(L)=0$. 
We solved this exact boundary value problem in Section \ref{stst:sectBVP}, where
...
$n$. Thus, for any $n \in \mathbb{N}_+$ we have the problem
\begin{eqnarray}
...
solution given in Equation(\ref{eq:soldifdibc}) we have at $t=0$
\begin{eqnarray}
...
\begin{question}
	Can you show that Equation (\ref{eq:soldifdibc}) is actually a solution to the diffusion equation with zero Dirichlet BCs?
\end{question}	
...
\noindent The result in Equation (\ref{eq:soldifdibc}) is a central result and is used in the more 
...
\end{eqnarray}
with BCs in Equation (\ref{eq:diffc0stst}). Using the method from Section \ref{sect:BV} we can 
...
\begin{eqnarray}
	c(x,t) = c_\text{st}(x) + \overline{c}(x,t)=ax+c_1 + \sum_{n=1}^\infty b_n
	\sin\left(\frac{n\pi}{L}x\right) e^{-(n\pi/L)^2 Dt} \, ,
\end{eqnarray}
where $b_n$ is found directly from Equation (\ref{eq:bnf}) 
\begin{eqnarray}
...
\begin{exerciseregion}
	...
	\begin{exercise}
		If you have not done so already, show that Equation (\ref{eq:soldifdibc})
	...
	\begin{exercise} \label{expl:diffneumann}
...
%
% Section on variable diffusion coefficient - could be an exercise/exploration
% \section{Variable diffusion coefficients}
%
...
We then apply \emph{unbounded domain} \index{Unbounded domain} BCs    
\begin{eqnarray}
...
\end{eqnarray}
and therefore we can change the index of the last term in Equation (\ref{eq:FEuler})
\begin{eqnarray}
...
\end{eqnarray}
where $c_n \in \mathbb{C}$ are the \emph{complex Fourier coefficients} \index{Fourier
series!coefficients} which 
...
usual interval $[-L;L]$.
\begin{theorem}
	If the complex Fourier series representation of $f:\mathbb{R} \rightarrow \mathbb{R}$ converges uniformly
	to $f$ on $\R$ and $f = F$ then
	\begin{eqnarray}
...
However, Equation (\ref{eq:cnunbound}) leads to another very powerful definition, namely, the
\emph{Fourier transform} \index{Fourier transform}
\begin{eqnarray}
...
\begin{proof}
	...
	For the third property we define the function $w(x,t) = f(x,t)e^{-ikx}$ and show that the right-hand side in Equation (\ref{eq:ftransprop3}) 
	...
	\end{eqnarray}
	which is the left-hand side of Equation (\ref{eq:ftransprop3}).	
\end{proof}
...
\end{eqnarray}
Substitution into Equation (\ref{eq:fouriersol}) we get 
\begin{eqnarray}
...
The example is not a coincidence, of course, and is generalized in the following statement: 
If $f: \R \rightarrow \R$ is piecewise continuous and bounded, then
\begin{eqnarray}
	\mathcal{F}^{-1}[\mathcal{F}[f]](x) = \mathcal{F}^{-1}[\fourier{f}](x) = f(x) \, .
\end{eqnarray}
This is the important \emph{inverse Fourier theorem}\index{Fourier transform!inverse theorem}. 

Equation (\ref{eq:fouriersolc}) is continous and bounded thus the inverse Fourier theorem qaranties that we recover $c$ by 
an inverse transform. We can rewrite Equation (\ref{eq:fouriersolc}) to 
\begin{eqnarray}
	\fourier{c}(k, t) = c_0 \sqrt{\pi} \sigma e^{-\alpha k^2} ,
\end{eqnarray}
with $\alpha = \frac{\sigma^2}{4} + Dt$. This is on the same form as Equation (\ref{eq:expx2F}), and we can simply substitue and get
\begin{eqnarray}
	\label{eq:solcfinal}
...
Equation (\ref{eq:initRealUnbounded}).

\begin{exerciseregion}

	\begin{exercise}
		Confirm Equation (\ref{eq:solcfinal}).
	\end{exercise}
	...
	\begin{exploration}
		...
		When can we use Equation (\ref{eq:solcfinal}) as an approximation to the solution 
		to this diffusion system? Solve Equation (\ref{eq:rdpol_2}) numerically using the 
