...
is parallel with the $x$-axis as shown in Fig. \ref{fig:flux}. Inside
...
Now, the first step in the exploration is to derive the underlying dynamical equation for $c$.
We focus on the volume confined between $x_1$ and $x_2$ and define the 
width $\Delta x = x_2 - x_1$. By the \emph{flux}\index{Flux} at $x_1$, denoted
...
\begin{figure}[h!]
	\begin{center}
		...
		Schematic illustration of the effective one dimensional tube system. The shaded area are the 
		tube cross-section area. See the text for explanations of the symbols. 
		}
	\end{center}
\end{figure}
...
\begin{eqnarray}
	\frac{\d N_S}{\d t} &=& \mathrm{production rate} + \mathrm{flux \ at} \ x_1 -
	\mathrm{flux \ at} \ x_2 \nonumber \\
	&=& \Delta \sigma + J_1 - J_2 \, . \label{eq:balance}
\end{eqnarray}
This equation is the \emph{balance equation} \index{Balance equation} for  $S$ in the volume. The amount of $S$ 
is related to the concentration $c$ by 
\begin{equation}
	N_S(t) = A \Delta x c(t) \ \ \mathrm{implying} \ \ \frac{\d N_S}{\d t} = A \Delta
	x \frac{\d c}{\d t} \, . 
\end{equation}
Substitution into Equation (\ref{eq:balance}) and dividing with the volume $A \Delta x$ we get
\begin{equation}
	\frac{\d c}{\d t} = \frac{\Delta \sigma}{A \Delta x} - \frac{1}{A} \frac{J_2
	-J_1}{\Delta x} \, .
\end{equation}
In the limit $\Delta x \rightarrow 0$ we get, now highlighting the time dependence 
\begin{equation}
	\frac{\d}{\d t} c(t) = f(t) - \frac{\d}{\d x}J(t) \, . 
\end{equation}
Here $f$ is the production rate of $c$ per unit volume inside the volume, and $J_s$ is
...
\begin{equation}
	...
	\,  .
\end{equation}
For the balance equation, Equation (\ref{eq:reacflux}), to be meaningful we assume
that both $c$ and $J$ have partial derivatives in their domain. We here referre to $f$ 
as the \emph{production function}\index{Production function}\index{Function!production} and $J$ simply as the \emph{flux}.
...
term due to \emph{diffusion}\index{Diffusion}. Advection is the process that transport a substance
...
If you add a drop of colored dye to a glass of water the dye molecules will
...
The RD-equation can be used to model many other systems than bio-chemical systems, and $c$ can
...
\begin{example}
	...
	are called \emph{Dirichlet boundaries} \index{Boundary!Dirichlet} or boundaries of first
	...
	Another commonly used BC is the \emph{Neumann boundary}\index{Boundary!Neumann}, or boundary of the second
...
\begin{example}
	...
	For sufficiently small flow rate or, equivalently small $v$, we have
	\begin{equation}
	...
	speed $V_0$ while keeping the other fixed. This gives Dirichlet BCs  
	\begin{equation}
...
\begin{example}
	...
	of $c_1$ and that the diffusion of $c_2$ only depedends on $c_2$ we can write a system of RD-equations 
	\begin{eqnarray}
...
\begin{example}
	...
	\end{equation}
	if $D$ is constant and if the production function is $r(c)$. Only a few
	exercises deal with Equation (\ref{eq:rdpol}).
\end{example}	
...
\noindent The \emph{steady state} \index{Steady state} is the state where the system is independent
...
In the next chapter we will explore these problems; both in the linear case and 
